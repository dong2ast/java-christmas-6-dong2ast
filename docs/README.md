...
1. **ENUM, 상수 등 기본 설정**
    ...
    - [x] 최대 주문 개수 20 (상수)
    - [x] 이벤트 적용 트리거 : 10000원 (상수)
    - [x] 혜택 금액에 따른 12월 이벤트 배지 (ENUM)
      - 5천원 이상 : 별
      - 1만원 이상 : 트리
      - 2만원 이상 : 산타
    - [x]  메뉴 이름 및 가격 설정 (ENUM)
      - [x] 에피타이저
...
2. **화면 입력**
    - [x] 식당 예상 방문 날짜 입력받기
        - [x] 1~31 사이의 값이 들어오지 않으면 예외 처리
    - [x] 주문할 메뉴와 개수 입력받기
      - [x] 메뉴판에 없는 메뉴를 입력하는 경우 예외 처리
      - [x] 주문 개수가 1 이상이 아닌 경우 예외 처리
...
#### Case 3. 주문 개수가 1 이상이 아닌 경우

#### Case 4. 메뉴 형식이 예시와 다른 경우

#### Case 5. 중복 메뉴를 입력한 경우

#### Case 6. 음료만 주문한 경우

#### Case 7. 한 번에 20개가 넘는 메뉴를 주문한 경우
